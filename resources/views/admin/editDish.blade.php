<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modification du plat : ' . $dish->name) }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-xl mx-auto sm:px-3 lg:px-4">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="overflow-x-auto relative my-4">
                    ...
                    @endif
                    <form method="POST" enctype="multipart/form-data" class='flex flex-col justify-center items-start p-8' action="{{ route('updateDish', ['restaurantId' => $restaurant, 'dishId' => $dish->id]) }}">
                        @csrf
                        <div class="flex justify-center items-center my-3">
                            <label for="name" class="mr-6 w-28">Nom</label>
                            <input type="text" name="name" id="name" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required value="{{ old('name') ?? $dish->name}}">
                        </div>
                        <div class="flex justify-center items-center my-3">
                            <label for="description" class="mr-6 w-28">Description</label>
                            <input type="text" name="description" id="description" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required value="{{ old('description') ?? $dish->description }}">
                        </div>
                        <div class="flex justify-center items-center my-3">
                            <label for="price" class="mr-6 w-28">Prix</label>
                            <input type="text" name="price" id="price" class="rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required value="{{ old('price') ?? $dish->price }}">
                        </div>
                        <div class="flex justify-center items-center my-3">
                        ...
                        <p> Image actuelle : </p>
                        <img class="max-w-28 max-h-28" src="{{Storage::url($dish->photo) }}">
